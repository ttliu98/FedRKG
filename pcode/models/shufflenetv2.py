...
class ShuffleNetV2(nn.Module):
    def __init__(self, net_size, dataset, save_activations=False):
        ...
        self.linear = nn.Linear(out_channels[3], self._decide_num_classes())
...
def shufflenetv2(conf, arch=None):
    net_size = (arch if arch is not None else conf.arch).replace("shufflenetv2-", "")
    assert net_size in {"0.5", "1", "1.5", "2"}

    if "cifar" in conf.data or "imagenet32" in conf.data:
        model = ShuffleNetV2(net_size=net_size, dataset=conf.data)
    else:

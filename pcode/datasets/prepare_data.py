...
from pcode.datasets.loader.knowledge_graph import RecommendationDS
...
def _get_recommendation(conf, root, split="train", test_ratio=0.2):
    is_train = split == "train"
    return RecommendationDS(conf.data, is_train)
...
def get_dataset(
    ...
    elif name in ["music", "movie","book"]:
        return _get_recommendation(conf, root, split)
    else:

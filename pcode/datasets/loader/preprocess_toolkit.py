...
def get_transform(
    ...
    if normalize is None and not is_downsampled:
        normalize = __imagenet_stats

    if "imagenet" in name:
        scale_size = scale_size or (
            downsampled_img_scale if downsampled_img_scale is not None else 256
        )
        input_size = input_size or (
            downsampled_img_scale if downsampled_img_scale is not None else 224
        )
